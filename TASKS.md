...
- [ ] **V-007** TUI: minimal textual app that connects to the local server, joins the same `"vertical-slice"` room, and shows the Yjs document's text content in a single read-only widget. (test: pytest integration test using `textual.pilot` confirms the widget renders text after a remote Yjs update is applied.)
